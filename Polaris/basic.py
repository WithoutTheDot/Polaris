    ...
    if not debug:
      if not res.error and self.current_tok.type != TT_EOF:
        return res.failure(InvalidSyntaxError(
          ...
          "Token cannot appear after previous tokens (Expected 'END' and/or uncompleted block)"
        ))
      return res
...
class BuiltInFunction(BaseFunction):
  ...
  def execute_str(self, exec_ctx):
    to_convert = exec_ctx.symbol_table.get("to_convert")

    try:
      return RTResult().success(String(str(to_convert.value)))
    except Exception as e:
      return RTResult().failure(RTError(
        self.pos_start, self.pos_end,
        "Invalid Type to convert to string",
        exec_ctx
      ))
  execute_str.arg_names = ["to_convert"]

  def execute_int(self, exec_ctx):
    to_convert = exec_ctx.symbol_table.get("to_convert")

    try:
      return RTResult().success(Number(int(to_convert.value)))
    except Exception as e:
      return RTResult().failure(RTError(
        self.pos_start, self.pos_end,
        "Invalid, failed to convert to integer",
        exec_ctx
      ))
  execute_int.arg_names = ["to_convert"]

  def execute_float(self, exec_ctx):
    to_convert = exec_ctx.symbol_table.get("to_convert")

    try:
      return RTResult().success(Number(float(to_convert.value)))
    except Exception as e:
      return RTResult().failure(RTError(
        self.pos_start, self.pos_end,
        "Invalid, failed to convert to float",
        exec_ctx
      ))
  ...
  def execute_insert(self, exec_ctx): 
    ...
    position = exec_ctx.symbol_table.get("position")
    if not isinstance(arr, List):
      return RTResult().failure(RTError(
        self.pos_start, self.pos_end,
        "First argument must be list",
    ...
    if not isinstance(position, Number) or position.value % 1 != 0:
      return RTResult().failure(RTError(
        self.pos_start, self.pos_end,
        "Third argument must be a integer",
        exec_ctx
      ))
  
    arr.elements.insert(position.value, item)
    return RTResult().success(Number.null)
  execute_insert.arg_names = ["arr", "item", "position"]
  ...
  def execute_replace(self, exec_ctx):
    ...
    if not isinstance(from_, String):
      return RTResult().failure(RTError(
        self.pos_start, self.pos_end,
        "Second argument must be a string",
        exec_ctx
      ))
    if not isinstance(to_, String):
      return RTResult().failure(RTError(
        self.pos_start, self.pos_end,
        "Third argument must be a string",
        exec_ctx
      ))
    return RTResult().success(String(string_.value.replace(from_.value, to_.value)))
    

  execute_replace.arg_names = ["string", "from", "to"]
  ...
  def execute_split(self, exec_ctx):
    ...
    return RTResult().success(List(split_value))
